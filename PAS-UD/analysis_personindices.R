...
setwd("C:/Users/User/Documents/PAS-UD")
###########
d = read_csv("args_newway_20200820.csv") 
d = d %>%
  ...
  mutate(subj_type_cat = case_when(grepl("modified", subj_type) ~ 5,
                                   grepl("det", subj_type) ~ 4,
                                   grepl("NOUN", subj_type) ~ 3,
                                   grepl("PROPN", subj_type) ~ 3,
                                   grepl("PRON", subj_type) ~ 2,
                                   grepl("Person", feats) & !grepl("Person\\[",feats) ~ 1,
                                   grepl("Person\\[erg\\]", feats) | (grepl("Person\\[abs\\]", feats) & !grepl("Person\\[erg\\]", feats)) ~ 1,
                                   is.na(subj_type) ~ 0,
                                   TRUE ~ 3),
         obj_type_cat = case_when(grepl("modified", obj_type) ~  5,
                                   grepl("det", obj_type) ~ 4,
                                   grepl("NOUN", obj_type) ~ 3, 
                                   grepl("PROPN", obj_type) ~ 3, 
                                   grepl("PRON", obj_type) ~ 2,
                                   grepl("Person\\[erg\\]", feats) & grepl("Person\\[abs\\]", feats)  ~ 1,
                                   is.na(obj_type) ~ 0,
                                   TRUE ~ 3))

d = d %>% mutate(has_obj = case_when(!has_obj & (obj_type_cat > 0) ~ T, T ~ has_obj))
...
ggplot(d.o, aes(x=as.factor(nom), y=diff)) + geom_jitter(width=.1) + 
  ...
  scale_x_discrete(labels = c("empty", "bound", "pronoun", "proper", "not modified", "modified")) +
